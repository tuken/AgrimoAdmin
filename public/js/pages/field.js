document.addEventListener('DOMContentLoaded', async () => {
  ...
  modalSaveBtn?.addEventListener('click', async () => {
    ...
    if (validationError) {
      window.appToast?.(validationError, 'error');
      return;
    }
    ...
    try {
      ...
      openModal(nextField);
      setEditing(false);
      window.appToast?.('圃場を更新しました。', 'success');
    } catch (err) {
      console.error('圃場更新に失敗しました:', err);
      window.appToast?.(err?.message || '圃場更新に失敗しました。', 'error');
    } finally {
  ...
  async function handleCreateFieldSave() {
    ...
    try {
      ...
      closeCreateModal();
      window.appToast?.('圃場を登録しました。', 'success');
    } catch (err) {
      console.error('圃場登録に失敗しました:', err);
      window.appToast?.(err?.message || '圃場登録に失敗しました。', 'error');
    } finally {
